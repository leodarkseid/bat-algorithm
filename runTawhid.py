...
def run_tawhid(benchmark, dim, n_bats, n_iter, lb, ub, seed=None, Sol_init=None ):
    if seed is not None:
        np.random.seed(seed)
    if Sol_init is None:
...
if __name__ == "__main__":
    ...
    run_tawhid(benchmark=benchmarks[selected], dim=int(dimension), n_bats=int(n_bats), n_iter=int(n_iter), lb=float(lb), ub=float(ub))